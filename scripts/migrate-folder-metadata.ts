...
const { folderMetadata } = require('../src/config/folder-metadata');

// Define the FolderMetadata interface to match our data structure
...
async function migrateFolderMetadata() {
  ...
  for (const [folderName, metadata] of Object.entries(folderMetadata) as [string, FolderMetadata][]) {
    try {
